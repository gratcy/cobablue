<?php
class Lostpwd_model extends CI_Model {
	...
	function __get_userid($email) {
		$this -> db -> select("uid FROM users_tab WHERE uemail='".$email."'");
	...
	function __check_key_exists($email,$key) {
		$this -> db -> select("b.tid FROM users_tab a INNER JOIN token_tab b ON a.uid=b.tuid WHERE b.ttype=2 AND a.uemail='".$email."' AND b.tkey='".$key."' AND b.tstatus=1");
		return $this -> db -> get() -> result();
	}
}
