<?php
$db['config']['database'] = true;
$db['config']['multiple'] = true;

$db['default']['dbhost'] = 'localhost';
$db['default']['dbuser'] = 'gratcy';
$db['default']['dbpassword'] = 'palma';
$db['default']['dbdatabase'] = 'bluenexia_api_db';
$db['default']['dbdriver'] = 'mysql';
$db['default']['dbdebug'] = true;
$db['default']['dbpersistant'] = false;
$db['default']['dbcharset'] = 'utf8';
$db['default']['dbcollat'] = 'utf8_general_ci';

$db['main']['dbhost'] = 'localhost';
$db['main']['dbuser'] = 'gratcy';
$db['main']['dbpassword'] = 'palma';
$db['main']['dbdatabase'] = 'bluenexia_db';
$db['main']['dbdriver'] = 'mysql';
$db['main']['dbdebug'] = true;
$db['main']['dbpersistant'] = false;
$db['main']['dbcharset'] = 'utf8';
$db['main']['dbcollat'] = 'utf8_general_ci';

$db['whmcs']['dbhost'] = 'localhost';
$db['whmcs']['dbuser'] = 'gratcy';
$db['whmcs']['dbpassword'] = 'palma';
$db['whmcs']['dbdatabase'] = 'bluenexia_db';
$db['whmcs']['dbdriver'] = 'mysql';
$db['whmcs']['dbdebug'] = true;
$db['whmcs']['dbpersistant'] = false;
$db['whmcs']['dbcharset'] = 'utf8';
$db['whmcs']['dbcollat'] = 'utf8_general_ci';
